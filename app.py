...
def inject_custom_css():
    st.markdown(
        ...
        :root { --primary-color: moccasin; --secondary-color: #1a1a1a; --text-main: #ffffff; }
        ...
        div[data-baseweb="input"] > div, div[data-baseweb="select"] > div { background-color: rgba(0, 0, 0, 0.45) !important; }
        input[type="number"], div[data-baseweb="select"] span { color: white !important; }
        div[data-testid="stFileUploader"] section { background-color: rgba(0, 0, 0, 0.45) !important; }
        .metric-card { background: rgba(255,255,255,0.05); border-radius: 16px; padding: 20px; }
        .stButton > button { background: linear-gradient(135deg, #D4FF00 0%, #aacc00 100%); color: black; }
...
def get_ai_recommendations(analysis: Dict, body_measurements: BodyMeasurements) -> Dict:
    ...
    try:
        ...
        return {"priority_areas": [], "diet": [f"❌ Gemini Client Initialization Error: {str(e)}"], "exercise": [], "lifestyle": []}
...
CRITICAL INSTRUCTION: Respond with STRICT JSON object ONLY. NO extra text.
...
### REGIONAL FAT ACCUMULATION SCORES ###
...
"""
    ...
    try:
        ...
        return {"priority_areas": [], "diet": [f"❌ Gemini API Call Failed: Status {e.status_code} - {e.message}"], "exercise": [], "lifestyle": []}
    except Exception as e:
        return {"priority_areas": [], "diet": [f"❌ Unknown Error during AI generation: {str(e)}"], "exercise": [], "lifestyle": []}
...
def calculate_bmi_metrics(height_cm: float, weight_kg: float, age: int, gender: str) -> Dict:
    ...
    return {"bmi": round(bmi, 2), "category": category, "risk": risk, "healthy_weight_range": (round(18.5 * height_m ** 2, 1), round(24.9 * height_m ** 2, 1)),}
...
def main():
    inject_custom_css()
    # Use images in the assets folder (relative paths) so Streamlit Cloud can find them
    set_bg("assets/background.jpg")

    with st.sidebar:
        st.markdown("## SEGMENT FIT TRAINER")
        selected = st.radio("Menu", ["Home", "Analysis", "Instructions", "About"], index=0, label_visibility="collapsed")
        st.markdown("---")
    ...
    if selected == "Home":
        ...
        with right:
            ...
            try:
                ...
                st.markdown(f'<img src="data:image/jpeg;base64,{img_base64}" style="width:350px;border-radius:50%">', unsafe_allow_html=True)
            except FileNotFoundError:
        ...
        if uploaded_file:
            ...
            body_measurements = BodyMeasurements(height_cm, weight_kg, age, gender, bmi_data["bmi"], bmi_data["category"]) if bmi_data else None
            if body_measurements is None:
                st.error("Invalid height/weight inputs")
            else:
                ...
                if annotated_img is None:
                    ...
                    for row in r_rows:
                        ...
                        for idx, (r_name, r_data) in enumerate(row):
                            with cols[idx]:
                                ...
                                st.markdown(f"<div class='metric-card' style='border-top: 3px solid {color}; padding: 4px;'><h4 style='margin:0; color:#fff; text-align:center;'>{r_name.replace('_',' ').upper()}</h4><h2 style='margin:0; font-size:1.5rem !important;text-align:center;'>{score:.2f}</h2><small style='color:{color}'>{level}</small></div>", unsafe_allow_html=True)
                    ...
                    if recommendations.get("priority_areas"):
                        ...
                        st.markdown(f"<div style='background: rgba(255, 68, 68, 0.15); border-left: 4px solid #ff4444; padding: 15px; border-radius: 8px; margin-bottom: 20px;'><strong style='color: #ff4444;'>⚠️ PRIORITY ZONES DETECTED:</strong> <span style='color: #fff;'>{p_str}</span></div>", unsafe_allow_html=True)
                    else:
                        st.success("✅ No critical high-fat zones detected. Focus on maintenance and performance.")
                    t1, t2, t3 = st.tabs(["🥗 NUTRITION PROTOCOL", "🏋️ TRAINING BLUEPRINT", "🧬 LIFESTYLE HACKS"])
                    with t1:
        ...
        st.markdown("<h2>ℹ️ ABOUT SYSTEM</h2>", unsafe_allow_html=True)
        st.markdown("""
            <div class='metric-card'>
...
<div class="footer"><p><strong>Segment Fit Trainer © 2025</strong></p><p>Empowering smarter fitness through AI-driven body analysis</p></div>
""", unsafe_allow_html=True)
